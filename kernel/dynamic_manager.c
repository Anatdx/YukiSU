...
void ksu_add_manager(uid_t uid, int signature_index)
{
	...
	if (!ksu_is_dynamic_manager_enabled()) {
		pr_info(
		    "Dynamic sign not enabled, skipping multi-manager add\n");
		return;
	}
...
static void clear_dynamic_manager(void)
{
	...
	for (i = 0; i < MAX_MANAGERS; i++) {
		if (active_managers[i].is_active) {
			pr_info("Clearing dynamic manager uid=%d "
				"(signature_index=%d) for rescan\n",
...
static void do_load_dynamic_manager(struct callback_head *_cb)
{
	...
	for (i = 0; i < 64; i++) {
		...
		if (!((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f'))) {
			pr_err("Invalid saved config hash character at "
			       "position %d: %c\n",
			       i, c);
			goto close_file;
		}
...
int ksu_handle_dynamic_manager(struct dynamic_manager_user_config *config)
{
	...
	case DYNAMIC_MANAGER_OP_SET:
		...
		for (i = 0; i < 64; i++) {
			...
			if (!((c >= '0' && c <= '9') ||
			      (c >= 'a' && c <= 'f'))) {
				pr_err("invalid hash character at position %d: "
				       "%c\n",
				       i, c);
				return -EINVAL;
			}
	...
	case DYNAMIC_MANAGER_OP_CLEAR:
		...
		strcpy(dynamic_manager.hash, "000000000000000000000000000000000"
		...
		pr_info(
		    "Dynamic sign config cleared (multi-manager disabled)\n");
		break;
