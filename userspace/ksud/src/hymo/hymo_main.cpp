    ...
    for (auto& op : plan.overlay_ops) {
        for (auto& layer : op.lowerdirs) {
            ...
            if (layer_str.find(mirror_str) == 0) {
                ...
                std::error_code ec_seg;
                if (fs::exists(layer, ec_seg) && !ec_seg) {
                    fs::create_directories(target.parent_path(), ec_seg);
                    if (!ec_seg) {
                        fs::rename(layer, target, ec_seg);
                    }
                    if (!ec_seg) {
...
int hymo::run_hymo_main(int argc, char** argv) {
    ...
    case Command::CONFIG: {
        ...
        if (subcmd == "gen") {
            ...
            std::cout << "  \"enable_stealth\": " << (config.enable_stealth ? "true" : "false")
                      << ",\n";
            std::cout << "  \"enable_hidexattr\": " << (config.enable_hidexattr ? "true" : "false")
                      << ",\n";
            ...
            std::cout << "  \"kasumi_available\": " << (Kasumi::is_available() ? "true" : "false")
                      << ",\n";
            ...
            std::cout << "  \"tmpfs_xattr_supported\": " << (check_tmpfs_xattr() ? "true" : "false")
                      << ",\n";
            std::cout << "  \"partitions\": [";
            ...
            const std::vector<std::string> candidates = scan_partition_candidates(config.moduledir);
    ...
    case Command::MODULE: {
        ...
        if (subcmd == "list") {
            ...
            if (rules.find(mod_id) != rules.end()) {
                ...
                auto it =
                    std::remove_if(mod_rules.begin(), mod_rules.end(),
            ...
            for (const auto& mod : module_list) {
                ...
                for (const auto& part : all_partitions) {
                    ...
                    {
                        std::error_code ec_conf;
                        auto it = fs::recursive_directory_iterator(part_dir, ec_conf);
                        auto end = fs::recursive_directory_iterator();
                        for (; it != end && !ec_conf; it.increment(ec_conf)) {
    ...
    case Command::API: {
        if (cli.args.empty()) {
            std::cerr << "Usage: ksud hymo api <system|storage|mount-stats|partitions|lkm|hooks>\n";
            return 1;
        }
        const std::string subcmd = cli.args[0];
    ...
    case Command::LKM: {
        ...
        if (lkm_subcmd == "load") {
            ...
            const bool on = (cli.args[1] == "on" || cli.args[1] == "1" || cli.args[1] == "true");
    ...
    if (can_use_kasumi) {
        ...
        if (Kasumi::set_enabled(config.kasumi_enabled)) {
            LOG_VERBOSE("Kasumi enabled=" + std::string(config.kasumi_enabled ? "true" : "false"));
        ...
        if (!storage.mode.empty()) {
            ...
            if (storage.mode == "erofs") {
                ...
                {
                    std::error_code ec_cln;
                    if (fs::exists(staging_dir, ec_cln)) {
                        fs::remove_all(staging_dir, ec_cln);
                    }
                    if (ec_cln) {
            ...
            LOG_ERROR("Failed to setup mirror storage.");
        }
        ...
        if (storage.mode == "erofs") {
            ...
            storage =
                setup_erofs_storage(mnt_base, staging_dir, fs::path(BASE_DIR) / "modules.erofs");
        } else {
    ...
    update_module_description(true, storage.mode, nuke_active,
                              ...
                              exec_result.magic_module_ids.size(), plan.kasumi_module_ids.size(),
                              warning_msg, kasumi_active);
